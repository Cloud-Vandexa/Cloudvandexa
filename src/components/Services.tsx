...
import { Users, Settings, LayoutGrid, Database, GraduationCap, HelpCircle, Share2 } from "lucide-react";
    ...
    <section className="py-24 bg-slate-50/50">
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-20">
          <h2 className="text-4xl md:text-5xl font-bold text-slate-800 mb-6">Exceptional Zoho Experience with CloudVandexa</h2>
          <div className="w-16 h-1 bg-zoho-blue mx-auto mb-10"></div>
          <p className="text-2xl text-gray-400 font-light max-w-4xl mx-auto italic">
            We are committed to delivering strategic solutions, seamless support, and a service experience that drives real business success.
          </p>
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {serviceList.map((s, i) => (
            <div key={i} className="bg-white p-10 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow flex flex-col h-full">
              <div className="text-zoho-blue mb-8 bg-zoho-blue/5 w-16 h-16 flex items-center justify-center rounded-xl">
                 {React.cloneElement(s.icon as React.ReactElement, { size: 32 })}
              </div>
              <h3 className="text-xl font-bold text-slate-800 mb-6 uppercase tracking-tight">{s.title}</h3>
              <p className="text-gray-500 font-light text-[17px] leading-relaxed">
                {s.desc}
              </p>
            </div>
          ))}
